...
/// <summary>
/// Item/currency color management category in the config window.
...
public class ItemsCategory
{
    ...
    public ItemsCategory(ConfigurationService configService, TrackedDataRegistry registry)
    {
    ...
    public void Draw()
    {
        ...
        if (ImGui.BeginTable("ItemColorsTable", 4, tableFlags, new Vector2(0, availableHeight)))
        {
            ...
            foreach (var categoryGroup in categories)
            {
                ...
                foreach (var definition in categoryGroup.OrderBy(d => d.DisplayName))
                {
                    ImGui.TableNextRow();

                    // Category column (only show for first item in category)
                    ImGui.TableNextColumn();
                    if (isFirstInCategory)
                    {
                        ImGui.TextUnformatted(categoryGroup.Key.ToString());
    ...
    private void SaveItemColor(TrackedDataType dataType, uint? color)
    {
        try
        {
            ...
            _configService.Save();
            LogService.Debug($"[ItemsCategory] Saved color for {dataType}: {color?.ToString("X8") ?? "(cleared)"}");
        }
        catch (Exception ex)
        {
