...
namespace Kaleidoscope.Gui.MainWindow.Tools.DataGraph;
...
/// Similar to DataTableTool but displays time-series data as a graph instead of a table.
/// </summary>
public class DataGraphTool : ToolComponent
{
    public override string ToolName => "Data Graph";
        ...
        FavoritesService? favoritesService = null)
    {
        ...
        if (textureProvider != null && trackedDataRegistry != null && favoritesService != null)
        {
            _currencyCombo = new CurrencyComboDropdown(
                ...
                trackedDataRegistry,
                favoritesService,
                "ItemGraphCurrencyAdd");
        }
    }
    
    /// <summary>
    /// Sets the series for this graph. Used by presets and conversion to pre-configure the graph.
    /// </summary>
    public void SetSeries(List<Gui.Widgets.ItemColumnConfig> series)
    {
        _instanceSettings.Series.Clear();
        _instanceSettings.Series.AddRange(series);
        _cacheIsDirty = true;
    }
    
    /// <summary>
    /// Configures graph settings. Used by presets and conversion to pre-configure display options.
    /// </summary>
    public void ConfigureSettings(Action<ItemGraphSettings> configure)
    {
    ...
    /// <summary>
    /// Gets the current series/items being tracked.
    /// </summary>
    public IReadOnlyList<Gui.Widgets.ItemColumnConfig> GetSeries() => _instanceSettings.Series;
    ...
    public override void RenderToolContent()
    {
        ...
        catch (Exception ex)
        {
            ...
            LogService.Debug($"[DataGraphTool] Draw error: {ex.Message}");
        }
    }
    ...
    private void DrawActionButtons()
    {
        ...
        {
            if (_currencyCombo.Draw(180))
            {
                if (_currencyCombo.SelectedType != default)
                {
                    AddSeries((uint)_currencyCombo.SelectedType, isCurrency: true);
    ...
    private List<(DateTime ts, float value)> GetCurrencyTimeSeries(ItemColumnConfig config, DateTime? startTime)
    {
        ...
        {
            LogService.Debug($"[DataGraphTool] GetCurrencyTimeSeries error: {ex.Message}");
            return new List<(DateTime ts, float value)>();
        }
    }
        ...
        Dictionary<string, List<(ulong characterId, DateTime timestamp, long value)>>? retainerBatchData)
    {
        ...
        {
            LogService.Debug($"[DataGraphTool] ExtractItemTimeSeries error: {ex.Message}");
            return new List<(DateTime ts, float value)>();
        }
    }
    ...
    private List<(DateTime ts, float value)> GetItemCurrentValue(ItemColumnConfig config, bool includeRetainers)
    {
        ...
        {
            LogService.Debug($"[DataGraphTool] GetItemCurrentValue error: {ex.Message}");
            return new List<(DateTime ts, float value)>();
        }
    }
        ...
        string baseName)
    {
        ...
        {
            LogService.Debug($"[DataGraphTool] GetCurrencyTimeSeriesPerCharacter error: {ex.Message}");
        }
        ...
        string baseName)
    {
        ...
        {
            LogService.Debug($"[DataGraphTool] ExtractItemTimeSeriesPerCharacter error: {ex.Message}");
        }
    ...
    public override void ImportToolSettings(Dictionary<string, object?>? settings)
    {
        ...
        if (settings.TryGetValue("Series", out var seriesObj) && seriesObj != null)
        {
            ...
            {
                LogService.Debug($"[DataGraphTool] Error importing series: {ex.Message}");
            }
        }
